...
export class AddNewNewsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getEmpDataList()
    this.defaultForm()
    if (currentUrl.includes('news-list')) {
      this.empId = "0";
      this.Isedit = false;
    } if (currentUrl.includes('edit-news-info')) {
      // debugger
  ...
  getEmpDataList() {
    ...
    this.adminLayoutService.getNewsMasterList().subscribe((empData: any) => {
      // debugger
  ...
  AddblogData() {
    ...
    for (var i = 0; i < this.myFiles.length; i++) {
      blogObj.append("videos", this.myFiles[i]);
    }
    ...
    this.SpinnerService.show();
    this.adminLayoutService.SaveNewsMaster(blogObj).subscribe((empData: any) => {
      // debugger
  ...
  UpdateEmpData() {
    ...
    if (this.deleteDId.length > 0) {
      ...
      blogObj.append("deletedVideos", this.PhotosId);
    }
    this.SpinnerService.show();
    // this.http.post(this.empDataUpdateApi, empModelObj).subscribe((empData: any) => {
    this.adminLayoutService.UpdateNewsMaster(blogObj).subscribe((empData: any) => {
      // debugger
      if (empData.meta.code == 200) {
        console.log(empData)
        this.defaultForm();
  ...
  editEmpData() {
    if (this.empId != "0") {
      ...
      this.Isedit = true;
      this.SpinnerService.show();
      this.adminLayoutService.getNewsMasterId(empId).subscribe((Response: any) => {
        // debugger
        ...
        this.urls = []
        for (var i = 0; i < Response.data.videosId.length; i++) {
          let ip = {
            videoUrl: this.imgUrl + Response.data.videosId[i].video_name,
            videoId: Response.data.videosId[i]._id,
          }
          this.urls.push(ip);
        }
        this.SpinnerService.hide();
